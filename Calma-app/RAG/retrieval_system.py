...
Este sistema permite buscar y recomendar los mejores especialistas  (OJO! SOLO ESPECIALISTAS)
(PARA RECURSOS Y TÉCNICAS ESTA KNOWLEDGE_RAG) basándose en:
...
class MentalHealthRetrieval:
    ...
    def __init__(self,
                 ...
                 index_path: str = 'datos/faiss_recursos/recursos_index.bin',
                 metadata_path: str = 'datos/faiss_recursos/recursos_metadata.pkl',
        ...
        # Caché de embeddings para queries comunes (reduce latencia)
    ...
    def search(self, 
        ...
        # Generar embedding de la query usando OpenAI con caché
        query_lower = query.lower().strip()
        
        if query_lower in self.query_embedding_cache:
            # Usar caché (ahorra 1-2 segundos)
            query_embedding = self.query_embedding_cache[query_lower]
        else:
            ...
            if len(self.query_embedding_cache) >= self.max_cache_size:
                ...
                first_key = next(iter(self.query_embedding_cache))
                del self.query_embedding_cache[first_key]
            
            self.query_embedding_cache[query_lower] = query_embedding
        
        # Normalizar para cosine similarity
        if query_embedding.shape == (1, 1536):
            faiss.normalize_L2(query_embedding)
        
        # Buscar en FAISS (OPTIMIZADO: menos candidatos para velocidad)
        k_search = min(top_k * 2, self.index.ntotal)  # Reducido de 3x a 2x
